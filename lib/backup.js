    ...
    try {
        ...
        if (typeof onProgress === "function") {
            onProgress({
                stage: "pruning",
                percent: 96,
                message: `Keeping latest ${MAX_BACKUPS_TO_KEEP} backups...`,
            });
        }

        try {
            deletedOldBackups = await pruneOldBackups(backupFolderId, MAX_BACKUPS_TO_KEEP);
        } catch (pruneError) {
...
async function listBackupFiles() {
    ...
    try {
        ...
        const response = await drive.files.list({
            ...
            spaces: "drive",
            orderBy: "modifiedTime desc",
            pageSize: MAX_BACKUPS_TO_DISPLAY,
