...
const MAX_BACKUPS_TO_DISPLAY = 10;
...
async function pruneOldBackups(folderId, keepCount = MAX_BACKUPS_TO_KEEP) {
    if (!oAuth2Client) {
        throw new Error("Not authorized. Please connect Google Drive first.");
    }
    ...
    do {
        ...
        allBackupFiles.push(...(response.data.files || []));
        pageToken = response.data.nextPageToken || null;
    } while (pageToken);

    if (allBackupFiles.length <= keepCount) {
        return 0;
    }

    const filesToDelete = allBackupFiles.slice(keepCount);
    const deleteResults = await Promise.allSettled(filesToDelete.map((file) => drive.files.delete({
        auth: oAuth2Client,
        fileId: file.id,
    })));
    ...
    if (failedCount > 0) {
        throw new Error(`Deleted ${deletedCount} old backups, but failed to delete ${failedCount}.`);
    }

    return deletedCount;
}

async function backupAppData(transactionsPath, auditTrailPath, uploadsPath, onProgress, maxBackupsToKeep = 10) {
    if (!oAuth2Client) {
        throw new Error("Not authorized. Please connect Google Drive first.");
    }
    ...
    try {
        ...
        if (typeof onProgress === "function") {
            onProgress({
                stage: "pruning",
                percent: 96,
                message: `Keeping latest ${maxBackupsToKeep} backups...`,
            });
        }

        try {
            deletedOldBackups = await pruneOldBackups(backupFolderId, maxBackupsToKeep);
        } catch (pruneError) {
            pruneWarning = String(pruneError?.message || "Failed to delete some old backups.");
        }
...
async function listBackupFiles(maxBackupsToDisplay = 10) {
    if (!oAuth2Client) throw new Error("Not authorized. Please connect Google Drive first.");
    try {
        const backupFolderId = await findOrCreateBackupFolder();
        const response = await drive.files.list({
...
async function restoreAppData(transactionsPath, auditTrailPath, uploadsPath, onProgress, fileId = "", maxBackupsToDisplay = 10) {
    if (!oAuth2Client) throw new Error("Not authorized. Please connect Google Drive first.");
    let tempZipPath = "";
    try {
        if (typeof onProgress === "function") {
