...
const DEFAULT_SETTINGS = {
    ...
    receiptPrefix: "",
    maxBackupsToKeep: 10,
    maxBackupsToDisplay: 10,
    compactUiMode: false,
    extraCompactUiMode: false,
};
...
function normalizeSettings(input = {}) {
    ...
    const receiptPrefix = String(input.receiptPrefix || "").trim();
    const maxBackupsToKeep = Number(input.maxBackupsToKeep);
    const maxBackupsToDisplay = Number(input.maxBackupsToDisplay);
    const compactUiMode = input.compactUiMode === true
        || input.compactUiMode === "true"
        || input.compactUiMode === "on";
    const extraCompactUiMode = input.extraCompactUiMode === true
        || input.extraCompactUiMode === "true"
        || input.extraCompactUiMode === "on";
    const safeCompactUiMode = compactUiMode || extraCompactUiMode;
    const safeExtraCompactUiMode = safeCompactUiMode && extraCompactUiMode;
    ...
    const safeReceiptPrefix = receiptPrefix;
    const safeMaxBackupsToKeep = Number.isFinite(maxBackupsToKeep) && maxBackupsToKeep > 0
        ? maxBackupsToKeep
        : DEFAULT_SETTINGS.maxBackupsToKeep;
    const safeMaxBackupsToDisplay = Number.isFinite(maxBackupsToDisplay) && maxBackupsToDisplay > 0
        ? maxBackupsToDisplay
        : DEFAULT_SETTINGS.maxBackupsToDisplay;

    return {
        annualTotalThreshold: safeThreshold,
        annualAlertPercent: safeAlertPercent,
        appBrand: safeAppBrand,
        receiptPrefix: safeReceiptPrefix,
        maxBackupsToKeep: safeMaxBackupsToKeep,
        maxBackupsToDisplay: safeMaxBackupsToDisplay,
        compactUiMode: safeCompactUiMode,
        extraCompactUiMode: safeExtraCompactUiMode,
    };
}
...
async function updateTransaction(id, transactionInput) {
    ...
    const newPaymentMethod = transactionInput.payment_method || "cash";
    let newPaymentReference = transactionInput.payment_reference || "";

    // If payment_method changes to 'cash', clear payment_reference
    if (existing.payment_method !== "cash" && newPaymentMethod === "cash") {
        newPaymentReference = "";
    }
...
async function renameCustomer(customerId, newName) {
    const normalizedCustomerId = String(customerId || "").trim();
    const normalizedNewName = String(newName || "").trim();

    if (!normalizedCustomerId || !normalizedNewName) {
        return { renamed: false, reason: "invalid-input" };
    }

    const transactions = await readTransactions();
    const affected = transactions.filter((t) => String(t.customer_id || "").trim() === normalizedCustomerId);

    if (affected.length === 0) {
        return { renamed: false, reason: "not-found" };
    }

    const oldName = String(affected[0].paidBy || "").trim();
    ...
    const updatedTransactions = transactions.map((t) =>
        String(t.customer_id || "").trim() === normalizedCustomerId
            ? { ...t, paidBy: normalizedNewName }
            : t,
    );
    await writeTransactions(updatedTransactions);

    const trail = await readAuditTrail();
    const updatedTrail = trail.map((entry) => {
        ...
        if (p.before && String(p.before.customer_id || "").trim() === normalizedCustomerId) {
            patch.before = { ...p.before, paidBy: normalizedNewName };
            changed = true;
        }

        if (p.after && String(p.after.customer_id || "").trim() === normalizedCustomerId) {
            patch.after = { ...p.after, paidBy: normalizedNewName };
            changed = true;
        }

        if (p.transaction && String(p.transaction.customer_id || "").trim() === normalizedCustomerId) {
            patch.transaction = { ...p.transaction, paidBy: normalizedNewName };
            changed = true;
        }

        return changed ? { ...entry, payload: { ...p, ...patch } } : entry;
    });
    await writeAuditTrail(updatedTrail);

    await appendAuditEntry("rename_customer", {
        customerId: normalizedCustomerId,
        before: { paidBy: oldName },
        after: { paidBy: normalizedNewName },
        affectedTransactionCount: affected.length,
    });

    return { renamed: true, oldName, newName: normalizedNewName, affectedCount: affected.length };
}
